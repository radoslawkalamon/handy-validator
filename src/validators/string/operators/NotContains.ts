...
export class NotContainsStringOperator implements IStringOperator {
  static errors = {
  ...
  public callback = (x: string, y: string): boolean => x.indexOf(y) === -1;
    ...
    const operatorConditions: IOperatorConditions[] = [
      {
        ...
        assumption: false,
        error: NotContainsStringOperator.errors.operatorArgumentsLengthInvalid,
      },
      {
        condition: this.isOperatorSecondArgumentString,
        assumption: false,
        error: NotContainsStringOperator.errors.operatorArgumentsTypesError,
      },
    ];
